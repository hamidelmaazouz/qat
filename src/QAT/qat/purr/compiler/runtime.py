...
class NewQuantumRuntime(QuantumRuntime):
    """
    ...
    def build_pass_pipeline(self):
        pm = PassManager()
        pm.add(PhaseOptimisation())
        pm.add(PostProcessingOptimisation())
        pm.add(CtrlHwValidation(self.engine))
        pm.add(PostProcessingValidation(self.model))
        return pm

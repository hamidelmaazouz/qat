...
class TriagePass(AnalysisPass):
    def run(self, builder: InstructionBuilder, res_mgr: ResultManager, *args, **kwargs):
        ...
        for inst in builder.instructions:
            ...
            if isinstance(inst, DeviceUpdate) or isinstance(inst, Pulse):
                names = (
                    [inst.value.name]
                    if isinstance(inst, DeviceUpdate) and isinstance(inst.value, Variable)
                        ...
                        [
                            var.name
                            for var in inst.__dict__.values()
    ...
    def run(self, builder: InstructionBuilder, res_mgr: ResultManager, *args, **kwargs):
        ...
        for target, instructions in triage_result.target_map.items():
            ...
            for inst in instructions:
                ...
                if isinstance(inst, Sweep):
                ...
                elif isinstance(inst, DeviceUpdate) or isinstance(inst, Pulse):
                    names = (
                        [inst.value.name]
                    ...
                    for name in names:
                        if not (
                            name in scoping_result.symbol2scopes
                            and [
                                (s, e)
                                for (s, e) in scoping_result.symbol2scopes[name]
                                if s in stack
                            ]
                        ):
                            raise ValueError(
                                f"Variable {name} referenced but no prior declaration found in target {target}"
                            )
                        rw_result.reads[name].append(inst)
            if stack:
                raise ValueError(
